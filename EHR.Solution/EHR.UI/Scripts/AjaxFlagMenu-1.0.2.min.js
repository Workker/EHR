    ...
    $.fn.AjaxFlagMenu =
        function (Options) {
            ...
            function AjaxFlagMenuItem(_Title, _onOutIcon, _onClickIcon, _HtmlStatusContent, _url, _data) {
                ...
                //  _AjaxFlagMenuItem.find("table").find("tr").append(HtmlStatusContent); // Detona o funcionamento
                ...
                _AjaxFlagMenuItem.mouseover(function () {
                    if (!_AjaxFlagMenuItem.hasClass(Options.onClickClass)) {
                        setAjaxFlagMenuItemClass(Options.onOverClass); _AjaxFlagMenuItem.uncorner();
                    }
                });
                ...
                _AjaxFlagMenuItem.click(function () {
                    if (!_AjaxFlagMenuItem.hasClass(Options.onClickClass)) {
                        setIcon(_onClickIcon); setAjaxFlagMenuItemClass(Options.onClickClass);
                        setHtmlStatusContentClass(Options.hscOutClass);
                        if (LastSelectedItem != null) {
                            LastSelectedItem.setOnOutStatus();
                        }
                        _AjaxFlagMenuItem.corner("4px");
                        var _XMLHTTPRequest = $.ajax({ type: "GET", url: _url, cache: false, data: _data }).success(function (d) {
                            $("#" + Options.ajaxContent).html(d);
                        });
                        ...
                        //            setHtmlStatusContentClass(Options.hscOutClass); HtmlStatusContent.find("div").corner("4px");
                        //        }
                        //    }
                        //};
                    }
            ...
            AjaxFlagMenu.mousemove(function () {
                for (var i = 0; i < iTems.length; i++) {
                    if (iTems[i].isClicked()) { LastSelectedItem = iTems[i]; break; }
                }
            });

            $(this).append(AjaxFlagMenu); return this;
        };
